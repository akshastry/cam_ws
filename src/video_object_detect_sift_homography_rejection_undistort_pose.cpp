      ...
      { 
        ...
        if(best_matches.size() >= 4)// no. of matches to use for pose estimation
        {
          ...
          if(!H.empty())
          {
            ...
            {
              Point2f diff = scene_feature_loc[best_matches[i].queryIdx] - keypoints_scene[ best_matches[i].trainIdx ].pt;
              if((unsigned int)mask.at<uchar>(i) && sqrt(diff.x*diff.x + diff.y*diff.y) < reprojectionError/2.0)
              {
                filtered_best_matches.push_back(best_matches[i]);
              }
            }
            ...
            if(isContourConvex(scene_corners))
            {
              ...
              vector<Mat> tvecs;
              vector<Mat> normals;
              decomposeHomographyMat  (H, K_matrix, rvecs, tvecs, normals);   
              rvec = rvecs[0];
              rvec = tvecs[0];
              ...
              // if(inliers_id.rows >=4 )
              // {
              //   vector<Point3d> obj_3D_inliers;
              //   vector<Point2d> scene_2D_inliers;
              //   for(int inliers_index = 0; inliers_index < inliers_id.rows; ++inliers_index)
              //   {
              //     int n = inliers_id.at<int>(inliers_index);         // i-inlier
              //     obj_3D_inliers.push_back(obj_3D[n]);
              //     scene_2D_inliers.push_back(scene_2D[n]);
              //   }

              //   // vector<Mat> rvecs;
              //   // vector<Mat> tvecs;
              //   // solvePnPGeneric( obj_3D_inliers, scene_2D_inliers, K_matrix/(2.0*scale), Mat::zeros(5, 1, CV_64FC1), rvecs, tvecs,
              //   //         useExtrinsicGuess, SOLVEPNP_IPPE, rvec, tvec);

              //   solvePnPRefineLM(obj_3D_inliers, scene_2D_inliers, K_matrix/scale, Mat::zeros(5, 1, CV_64FC1), rvec, tvec,
              //           TermCriteria(TermCriteria::EPS+TermCriteria::COUNT, 100, FLT_EPSILON) );
                ...
                vector<Point2f> scene_corners(4);
                projectPoints(obj_corners, rvec, tvec, K_matrix/scale, Mat::zeros(5, 1, CV_64FC1), scene_corners);
                ...
                cout << "No. of solutions: " << rvecs.size() << "\n";
