...
int main( int argc, char** argv ){
  ...
  vector<Point3f> obj_feature_loc_3D;
  img_width_length = 25.8; // cm
  img_height_length = 14.6; // cm
  ...
  while(1){
    ...
    if(best_matches.size() >= 4)// no. of matches to use for pose estimation
    {
      ...
      { 
        ...
        {
          //-- Localize the object
          vector<Point3f> obj_3D;
          vector<Point2f> scene_2D;
          ...
          ////TODo: check the output of solve pnp, remove one of the result and then draw axes
