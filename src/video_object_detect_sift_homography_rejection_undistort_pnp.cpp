...
int main( int argc, char** argv ){
  ...
  cap.set(cv::CAP_PROP_FRAME_WIDTH, 1280);// calibration is with this value
  cap.set(cv::CAP_PROP_FRAME_HEIGHT, 720);// calibration is with this value
  ...
  while(1){
    ...
    Mat final_frame;
    resize(rectified_frame, img_scene, Size(img_width/2, img_height/2), INTER_AREA);
    scale = 1;
      ...
      { 
        ...
        if(filtered_best_matches.size() >= 4)// no. of matches to use for pose estimation
        {
          ...
          for( size_t i = 0; i < filtered_best_matches.size(); i++ )
          {
            ...
            scene_2D.push_back( keypoints_scene[ filtered_best_matches[i].trainIdx ].pt * (1.0*scale) );
          }

          solvePnPRansac( obj_3D, scene_2D, K_matrix/2.0, Mat::zeros(5, 1, CV_64FC1), rvec, tvec,
          ...
          projectPoints(axes, rvec, tvec, K_matrix/2.0, Mat::zeros(5, 1, CV_64FC1), axes2D);
          for (int i = 0; i < 4; i++)
          ...
          projectPoints(obj_corners, rvec, tvec, K_matrix/2.0, Mat::zeros(5, 1, CV_64FC1), scene_corners);
          for (int i = 0; i < 4; i++)
