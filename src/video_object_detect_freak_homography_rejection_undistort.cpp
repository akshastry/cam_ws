...
int main( int argc, char** argv ){
  ...
  while(1){
    ...
    for( int i = 0; i < matches12.size(); i++ )
    { 
      if( matches12[i].size()>=2 && matches12[i][0].distance <= 2*min_dist12 && matches12[i][0].distance < matches12[i][1].distance * 0.9)
      { good_matches12.push_back( matches12[i][0]); }
    }

    for( int i = 0; i < matches21.size(); i++ )
    { 
      if( matches21[i].size()>=2 && matches21[i][0].distance <= 2*min_dist21 && matches21[i][0].distance < matches21[i][1].distance * 0.9)
